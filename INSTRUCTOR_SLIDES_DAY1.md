...
• WiFi: [Network name]
• Restrooms: [Location]
• Breaks: As needed (flexible)
• Questions: Anytime - interrupt me!
• Materials: Website accessible throughout
...
**Design Notes:**
- Left side: Text
- Right side: Visual comparison (vague → precise)
...
User: "Summarize this paper"        →   [AUTOMAT structured prompt]
AI: Generic summary                     
User: "I need parameters"            →   First-shot success
AI: Inconsistent format                  • Precise output
User: "Make it a table"              →   • Correct format
AI: Missing columns                      • Complete data
User: "Add temp/pressure"            →   • Ready to use
AI: Mixed units
...
• Literature review: 8 hours
• Protocol formatting: 30 min/protocol
• Experimental brainstorming: 2 hours

After Prompt Engineering:
• Literature review: 2 hours (75% ↓)
• Protocol formatting: 5 min (83% ↓)
• Experimental brainstorming: 30 min (75% ↓)
...
→ R&D Colleagues: Technical, assume domain knowledge
→ Management: High-level, business impact focus
→ External Partners: Balanced, avoid proprietary info
```
...
Why it matters:
Without persona → "Helpful generalist" (too broad)
With persona → Domain expert (targeted depth)
...
• Markdown table with specified columns
...
- Arrow showing method → process → output
...
✓ Functional tasks with clear outputs
...
❌ Conversational learning (use CO-STAR - Day 2)
...
**Content:**
```
Transactional ❌ → Learning Partnership ✅

Transactional:
"Summarize this paper" → [Copy summary] → Done
Result: Summary obtained, no learning

Learning Partnership:
"Summarize this paper" → [Read summary]
...
1. Unpublished Research Data
   ...
   • Exact synthesis parameters (proprietary)
   • Experimental results (ongoing R&D)
...
Ti₆Al₄V at 400°C with 2% oxygen?"
...
AI does:
✅ Predict most probable tokens
✅ Synthesize plausible-sounding answer
...
• Demand "Not Reported" over estimation
• Flag missing data, don't fill gaps
```
...
☐ Abstract aligns with claimed content
...
**Speaker Notes:**
> Verification is non-negotiable. For citations: verify DOI works, check authors match, read the abstract to confirm AI's claim aligns. For numbers: sanity check (PLA melting at 300°C? Impossible). Check units, compare to benchmarks. Use CrossRef for DOIs, Google Scholar for quick checks, your handbooks for reference values. Most importantly, apply your domain expertise. If it sounds wrong, it probably is. Trust but verify becomes: don't trust, always verify.
...
❌ Specific: "25kV, 15cm, 12% PLA (Mw 100kDa) 
in DMF:DCM (3:1), 5% graphene, 80°C, Yield: 87%"

✅ Sanitized: "[VOLTAGE], [DISTANCE], 
...
❌ "Batch #343: 45.2 MPa at 23°C"
✅ "Strength decreases ~15% between 20-60°C"
...
How it Works:
• Ollama + Open WebUI running on instructor laptop
• Llama 3.2 3B model (fully local)
• Connected via local WiFi (no internet)
...
• Extracts key synthesis parameters
• Outputs structured format for comparison
• Includes constraints for consistency
...
Common Mistakes:
❌ Vague Task: "Extract info from papers"
✅ Specific: "Extract synthesis temp, voltage, 
   graphene %, yield from Methods sections"

❌ Undefined Output: "Make it a table"
✅ Specific: "Markdown table with columns: 
   [DOI, Method, Temp (°C), Voltage (kV)...]"

❌ No Constraints: AI guesses when data missing
✅ Explicit: "If parameter not reported, mark 'NR'"
...
• 1 non-existent characterization technique
...
→ DOI format wrong, doesn't resolve
→ Verify: CrossRef search

Error 2: Physically Impossible
Simultaneous modulus AND ductility increase
→ Typically trade-off (stiffness ↔ flexibility)
→ Verify: Materials science principles

Error 3: Non-existent Technique
"Quantum-resolution XPS"
→ No such characterization method exists
...
✅ Green: Safe for external AI
⚠️ Yellow: Safe after sanitization
🚫 Red: Local sandbox only
...
→ Local sandbox OR sanitize image (remove metadata)
...
→ Local sandbox OR sanitize (remove names/numbers)
```
...
☐ Review Day 1 content on website
  (Especially AUTOMAT examples)

☐ Try one AUTOMAT prompt in your real work
...
• Website: [URL]
• Instructor: [Email]
• Sandbox access: Available during all sessions
